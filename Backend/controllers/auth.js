...
exports.login = async (req, res) => {
  ...
  console.log('stored password:',storedUser.Password)
    ...
    res.status(200).json({  message: ' O You welcome you have been loged iiin!',token: token, userId: storedUser.id });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
console.log( 'post on /Nannys/nannyform' ,req.body)
  ...
  const Email = req.body.Email;
   console.log("req after nanny from" ,LastName,FirstName,Age,About,Region,Phonenumber,Email )
  ...
  res.status(201).json({ message: 'Nanny registered!',resultNannyregisterted });
}catch (err) {
    res.status(500).json({ error: err.message });
  }
};
